...
regannot[[i]] <- annot(chrannot[[i]], region) %>% rowwise() %>% 
    ...
    distinct(seqnames, start, end, n_sites, ave_pwd_N1N2, ave_pwd_JAJB, ave_pwd_IAIB, ave_pwd_DADB, ave_pwd_HAHB, ave_pwd_MAMB,ave_pwd_ENJN,ave_pwd_ENIN, genecat, annot.regid) 
...
ck <- dat %>% group_by(n_sites, genecat) %>% 
  mutate(N1N2=mean(ave_pwd_N1N2),
    ...
    distinct(n_sites, genecat, N1N2, JAJB, IAIB, DADB, HAHB, MAMB,ENJN,ENIN) %>% 
...
comparelist <- list(wgdf14pwd, wgdf23pwd, wgdf56pwd, wgdf12pwd, wgdf34pwd, wgdf56npwd, ENJNpwd, ENINpwd)
namelist <- list("N1N2", "JAJB", "IAIB", "DADB", "HAHB", "MAMB", "ENJN", "ENIN")
