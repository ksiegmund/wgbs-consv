...
aclist <- list()
for (i in 1:7){
  wktmp <- import.bw(sprintf("wgEncodeBroadHistone%sH3k4me1StdSig.bigWig", celllist[[i]]))
...
head(acfinal[[1]])
saveRDS(acfinal, file = "H3K4me1_data.Rds")
...
datalist <- readRDS("H3K4me1_data.Rds")
...
write_csv(ckdata, "H3K4me1_wgdata.csv")

ckdata <- read_csv("H3K4me1_wgdata.csv")
# 6510 bins;
# remove duplicate, make wide form data
wkck <- ckdata %>% dplyr::select(-annot.type, -annot.gene_name) %>% distinct()
# 4910
write_csv(wkck, "H3K4me1_wide_unique.csv")
...
write_csv(longdataf, "H3K4me1_long_unique.csv")
...
ckdata <- bind_rows(comblist)
write_csv(ckdata, "allchrH3K4me1_data.csv")

```

```{r mkplot, echo=FALSE, warning=FALSE}
# make plots by chromosome
ckdata <- read.csv("allchrH3K4me1_data.csv", header=TRUE, stringsAsFactors=FALSE)
ckdata %>% 
