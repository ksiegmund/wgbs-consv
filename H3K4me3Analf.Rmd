...
aclist <- list()
for (i in 1:7){
  wktmp <- import.bw(sprintf("wgEncodeBroadHistone%sH3k4me3StdSig.bigWig", celllist[[i]]))
...
head(acfinal[[22]])
saveRDS(acfinal, file = "H3K4me3_data.Rds")
...
write_csv(ckdata, "H3K4me3_wgdata.csv")

ckdata <- read_csv("H3K4me3_wgdata.csv")
# 40306 bins total
# remove duplicate, make wide form data
wkck <- ckdata %>% dplyr::select(-annot.gene_name, -annot.type) %>% distinct()
# 20587
write_csv(wkck, "H3K4me3_wide_unique.csv")
...
write_csv(longdataf, "H3K4me3_long_unique.csv")
...
ckdata <- bind_rows(comblist)
write_csv(ckdata, "allchrH3K4me3_data.csv")

```

```{r mkplot, echo=FALSE, warning=FALSE}
# make plots by chromosome
ckdata <- read.csv("allchrH3K4me3_data.csv", header=TRUE, stringsAsFactors=FALSE)
ckdata %>% 
