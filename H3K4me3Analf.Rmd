...
# read H3K4me3 data and liftover from hg19 to hg38 
...
aclist <- list()
for (i in 1:7){
  wktmp <- import.bw(sprintf("../H3K4me3/wgEncodeBroadHistone%sH3k4me3StdSig.bigWig", celllist[[i]]))
...
# annotate chr 100bp bins with H3K4me3 feature in each cell line, and calculate average H3K4me3 scores across the 7 cell lines for each bin
...
head(acfinal[[22]])
saveRDS(acfinal, file = "../H3K4me3/H3K4me3_data.Rds")
...
datalist <- readRDS("../H3K4me3/H3K4me3_data.Rds")
...
write_csv(ckdata, "../H3K4me3/H3K4me3_wgdata.csv")

ckdata <- read_csv("../H3K4me3/H3K4me3_wgdata.csv")
# 40306 bins total
# remove duplicate, make wide form data
wkck <- ckdata %>% dplyr::select(-annot.gene_name, -annot.type) %>% distinct()
# 20587
write_csv(wkck, "../H3K4me3/H3K4me3_wide_unique.csv")
...
write_csv(longdataf, "../H3K4me3/H3K4me3_long_unique.csv")
...
longdataf %>% 
  ...
  #facet_wrap(facets = ~reorder(seqnames, rank), ncol=6) +
    xlab("\n Average beta") +
     ylab("Average pwd\n") +
      ggtitle("High H3K4me3 score, CpG rich region\n") + 
...
ckdata <- bind_rows(comblist)
write_csv(ckdata, "../H3K4me3/allchrH3K4me3_data.csv")

```

```{r mkplot, echo=FALSE, warning=FALSE}
# make plots by chromosome
ckdata <- read.csv("../H3K4me3/allchrH3K4me3_data.csv", header=TRUE, stringsAsFactors=FALSE)
ckdata %>% 
  ...
  facet_wrap(facets = ~reorder(seqnames, rank), ncol=6) +
    xlab("\n Average pwd of non-gene CpGs") +
     ylab("Average pwd of CpG rich, high H3K4me3 score region\n") +
      ggtitle("High H3K4me3 score, CpG rich region\n") + 
       guides(color=guide_legend(" "))
...
datalist <- readRDS("H3K4me3_data.Rds")
...
write_csv(celldata, sprintf("%s_H3K4me3data.csv", scorelist[[j]]))
}
