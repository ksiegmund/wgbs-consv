...
# read H3K27ac data and liftover from hg19 to hg38 
...
aclist <- list()
for (i in 1:7){
  wktmp <- import.bw(sprintf("../H3K27ac/wgEncodeBroadHistone%sH3k27acStdSig.bigWig", celllist[[i]]))
...
# annotate chr 100bp bins with H3K27ac feature in each cell line, and calculate average H3K27ac scores across the 7 cell lines for each bin
...
head(acfinal[[1]])
summary(acfinal[[1]])
saveRDS(acfinal, file = "../H3K27ac/H3K27ac_data.Rds")
...
datalist <- readRDS("../H3K27ac/H3K27ac_data.Rds")
...
write_csv(ckdata, "../H3K27ac/H3K27ac_wgdata.csv")

ckdata <- read_csv("../H3K27ac/H3K27ac_wgdata.csv")
# 12970 bins total

# remove duplicate, make wide form data
wkck <- ckdata %>% dplyr::select(-annot.type, -annot.gene_name) %>% distinct()
# 6248
write_csv(wkck, "../H3K27ac/H3K27ac_wide_unique.csv")
...
write_csv(longdataf, "../H3K27ac/H3K27ac_long_unique.csv")
...
longdataf %>% 
  ...
  #facet_wrap(facets = ~reorder(seqnames, rank), ncol=6) +
    xlab("\n Average beta") +
     ylab("Average pwd\n") +
      ggtitle("High H3K27ac score, CpG rich region\n") + 
...
ckdata <- bind_rows(comblist)
write_csv(ckdata, "../H3K27ac/allchrH3K27ac_data.csv")

```

```{r mkplot, echo=FALSE, warning=FALSE}
# make plots by chromosome
ckdata <- read.csv("../H3K27ac/allchrH3K27ac_data.csv", header=TRUE, stringsAsFactors=FALSE)
#nongene <- ckdata %>% distinct(name, nongene)
#write_csv(nongene, "nongene.csv")
ckdata %>% 
  ...
  facet_wrap(facets = ~reorder(seqnames, rank), ncol=6) +
    xlab("\n Average pwd of non-gene CpGs") +
     ylab("Average pwd of CpG rich, high H3K27ac score region\n") +
      ggtitle("High H3K27ac score, CpG rich region\n") + 
       guides(color=guide_legend(" "))
...
write_csv(celldata, sprintf("%s_H3K27acdata.csv", scorelist[[j]]))
}
