...
aclist <- list()
for (i in 1:7){
  wktmp <- import.bw(sprintf("wgEncodeBroadHistone%sH3k27acStdSig.bigWig", celllist[[i]]))
...
head(acfinal[[1]])
summary(acfinal[[1]])
saveRDS(acfinal, file = "H3K27ac_data.Rds")
...
datalist <- readRDS("H3K27ac_data.Rds")
...
write_csv(ckdata, "H3K27ac_wgdata.csv")

ckdata <- read_csv("H3K27ac_wgdata.csv")
# 12970 bins total

# remove duplicate, make wide form data
wkck <- ckdata %>% dplyr::select(-annot.type, -annot.gene_name) %>% distinct()
# 6248
write_csv(wkck, "H3K27ac_wide_unique.csv")
...
write_csv(longdataf, "H3K27ac_long_unique.csv")
...
ckdata <- bind_rows(comblist)
write_csv(ckdata, "allchrH3K27ac_data.csv")

```

```{r mkplot, echo=FALSE, warning=FALSE}
# make plots by chromosome
ckdata <- read.csv("allchrH3K27ac_data.csv", header=TRUE, stringsAsFactors=FALSE)
