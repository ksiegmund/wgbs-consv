...
hmmdata[[i]] <- hmmfinal[[i]] %>% 
  ...
  distinct(seqnames, Names, n_bins, N1N2, JAJB, IAIB, DADB, HAHB, MAMB, ENIN, ENJN) %>% 
   left_join(state, by=c("Names"="grp")) %>% 
     mutate(statename=paste0(Names, "_", type)) %>% 
      arrange(Names) %>% pivot_longer(N1N2:ENJN) %>% 
...
hmmdata_all <- hmmdata_wg %>% group_by(Names) %>% 
  ...
  distinct(Names, n_bins, ave_n_sites, var_n_sites, min_n_sites, max_n_sites, N1N2, JAJB, IAIB, DADB, HAHB, MAMB,ENIN,ENJN) %>% 
   left_join(state, by=c("Names"="grp")) %>% 
     mutate(statename=paste0(Names, "_", type)) %>% 
      arrange(Names) %>% pivot_longer(N1N2:ENJN) %>% 
       left_join(comb, by = "name" ) 
