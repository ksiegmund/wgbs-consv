...
# essential gene list
 depmaplist <- read_excel("depmaplist.xlsx", sheet = 1)
...
expdata <- read_excel("SUMMARY OF KOREAN SINGLE CELL EXPRESSION COLON GSE132257.xlsx") 
...
write_csv(depmap_pwdall, "depmap_pwdall.csv")

# Remove chrX & chrY
ckdata <- read_csv("depmap_pwdall.csv") %>% filter(seqnames!="chrX" & seqnames!="chrY") 

table(ckdata$seqnames)

length(unique(ckdata$annot.gene_name))

write_csv(ckdata, "depmap_pwdall.csv")
...
write_csv(depmap_pwdall, "depmap_pwdall_long.csv")

# Remove chrX & chrY
ckdata <- read.csv("depmap_pwdall_long.csv") %>% filter(seqnames!="chrX" & seqnames!="chrY") 

table(ckdata$seqnames)

write_csv(ckdata, "depmap_pwdall_long.csv")
...
wktmp <- read.csv("depmap_pwdall.csv") %>% filter(grp=="essential")
...
colnames(wk) <- c("N1N2", "JAJB", "IAIB", "DADB", "HAHB", "MAMB")
