...
# read ctcf data and liftover from hg19 to hg38 

celllist <- list("Gm12878", "H1hesc", "Hsmm", "Huvec", "K562", "Nhek", "Nhlf")

#download.file("http://hgdownload.cse.ucsc.edu/gbdb/hg19/liftOver/hg19ToHg38.over.chain.gz",
...
ctcflist <- list()
for (i in 1:7){
  wktmp <- import.bw(sprintf("../CTCF/wgEncodeBroadHistone%sCtcfStdSig.bigWig", celllist[[i]]))
  ctcflist[[i]] <- as.data.frame(liftOver(wktmp, chainObject1))
}
...
# annotate chr 100bp bins with ctcf feature in each cell line, and calculate average ctcf scores across the 7 cell lines for each bin
...
# final merge
ctcffinal[[i]] <- chrannot[[i]] %>% left_join(mergescore, by=c("seqnames", "start", "end", "regid")) 
  
}

head(ctcffinal[[22]])
saveRDS(ctcffinal, file = "../CTCF/ctcf_data.Rds")

```

```{r wgdata, echo=FALSE, warning=FALSE}

# filtered for ave_score>100 & #CpGs>=5 bins

datalist <- readRDS("../CTCF/ctcf_data.Rds")
titlelist <- list("N1N2", "JAJB", "IAIB", "DADB", "HAHB", "MAMB","ENJN","ENIN")
comblist <- list()
for(i in 1:22){
  comblist[[i]] <- datalist[[i]] %>% filter(ave_score>100) %>% 
    rowwise() %>% 
...
write_csv(ckdata, "../CTCF/ctcf_wgdata.csv")

# remove duplicate, make wide form data
ckdata <- read_csv("ctcf_wgdata.csv")
# 11511 bins total
wkck <- ckdata %>% dplyr::select(-annot.type, -annot.gene_name) %>% distinct()
# 7991
write_csv(wkck, "../CTCF/ctcf_wide_unique.csv")
...
write_csv(longdataf, "../CTCF/ctcf_long_unique.csv")
...
datalist <- readRDS("ctcf_data.Rds")
comblist <- list()
for(i in 1:22){
  comblist[[i]] <- mkdata(datalist[[i]], sprintf("chr%s", i), 100)
  comblist[[i]]$rank <- i
}

comblist[[1]]
ckdata <- bind_rows(comblist)
write_csv(ckdata, "../CTCF/allchrCtcf_data.csv")

```

```{r mkplot, echo=FALSE, warning=FALSE, eval=FALSE}
# make plots by chromosome
ckdata <- read.csv("allchrCtcf_data.csv", header=TRUE, stringsAsFactors=FALSE)
ckdata %>% 
  ...
  facet_wrap(facets = ~reorder(seqnames, rank), ncol=6) +
    xlab("\n Average pwd of non-gene CpGs") +
     ylab("Average pwd of CpG rich, high CTCF score region\n") +
      ggtitle("High CTCF score, CpG rich region\n") + 
...
write_csv(celldata, sprintf("%s_Ctcfdata.csv", scorelist[[j]]))
}
