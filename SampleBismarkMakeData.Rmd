...
metdata_s1 <- fread("~/bismarkData/bed/FA1.bed", header=F)
colnames(metdata_s1) <- c("chr", "start", "end", "beta", "coverage", "num_met", "num_unmet")

metdata_s2 <- fread("~/bismarkData/bed/FB2.bed", header=F)
...
df_pwd <- gendf(joins)
summary(df_pwd)
write.table(df_pwd, "~/bismarkData/bed/FAFBpwd.bed", row.names = F, col.names = F, quote = F, sep = "\t")
